...
const banner = cfonts.render(("KING REST APIS"), {
font: "block",
align: "center",
...
app.get('/comment/add/:username/:comment', (req, res) => {
    const { username, comment } = req.params;
    const id = comments.length + 1;
    comments.push({ id, username, comment });
    fs.writeFileSync('comments.json', JSON.stringify(comments, null, 2));
    res.json({ message: 'Comentário adicionado', id });
});
...
var ITC = key.map(i => i?.apikey)?.indexOf(apikey);
return res.json({resultado: `ApiKey: ${apikey} 
Foi Adicionada ao Sistema\n🚀

Numero de Requisições 

Disponiveis: ${key[ITC]?.request}`})
}
})
...
await listkeys(apikey, req);
url = req.query.url
 if (!url) return res.json({ status : false, criador : `GOJO BOT & GOJO MODZ`, mensagem : "Coloque o parametro: url"})
fbdown(url).then(data => {
res.json({
status: true,
mensagem: 'KING REST APPIS - MELHOR SITE DE APIS',
criador: `KingDev`,
resultado: data
})}).catch(e => {
res.json({
...
res.json({
status: true,
mensagem: 'KING REST APIS - MELHOR SITE DE APIS',
criador: `KingDev`,
...
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);
if (!q) return res.json({ status : false, creator : `KingDev`, message : "Cade o parametro q?"}) 
...
await listkeys(apikey, req);
url = req.query.url
 if (!url) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: url"})
...
await listkeys(apikey, req);
q = req.query.q
 if (!q) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: q"})
...
await listkeys(apikey, req);
nome = req.query.nome
 if (!nome) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: nome"})
...
await listkeys(apikey, req);
signo = req.query.signo
if (!signo) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: signo"})
...
categoria = req.query.categoria
 if (!categoria) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: categoria"})
...
pergunta = req.query.pergunta
 if (!pergunta) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: pergunta"})
...
texto = req.query.texto
 if (!texto) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: texto"})
...
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);
  if (!query) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: query"})
...
await listkeys(apikey, req);
var ITC = key.map(i => i?.apikey)?.indexOf(apikey);
console.log(color(' │ APITOKEN:'  + ` ${apikey} • LIMITE: ${key[ITC]?.request}\n`,'red'),color('│','red'), color('REQUEST', 'red'), '• ' + color('PING: ' + ping(), 'red'))
var { YTNomeSearch } = require('./modulos-api/youtoba.js')
try {
...
if (!url) return res.json({ status : false, creator : `KingDev`, message : "Cade o parametro url?"}) 
        if(key.includes(apikey)){
       igstory(url)
	.then(data => {
		var result = data;
...
await listkeys(apikey, req);
text = req.query.text
if (!text) return res.json({ status : false, creator : `KingDev`, message : "Cade o parametro text?"})
        if(key.includes(apikey)){
...
if(!key.includes(apikey))return res.json({status:false,msg:'Apikey Inválida, entre em contato: wa.me/557791009651'})
	if(key.includes(apikey)){    
...
await listkeys(apikey, req);
query = req.query.query
if (!query) return res.json({ status : false, creator : `KingDev`, message : "Cade o parametro query?"})
wikimedia(query)
.then(hasil => {
var resultado = hasil;
res.json({
...
await listkeys(apikey, req);
url = req.query.url
if (!url) return res.json({ status : false, criador : `KingDev`, mensagem : "Coloque o parametro: q"})
...
res.json({
status: true,
mensagem: 'KING APIS - MELHOR SITE DE APIS',
criador: `KingDev`,
resultado: `${auu.str}`,
})
} catch (error) {
...
if(!texto)return res.json({status:false,motivo:'Cadê o parâmetro img?'})
    hasil = `https://api.bronxyshost.com.br/api-bronxys/attp_edit?texto=${encodeURIComponent(q)}&fonte=${Fontes}&apikey=BLUX_api_bot`
	  data = await fetch(hasil).then(v => v.buffer())   
         await fs.writeFileSync(bla+'/tmp/gostosinha.jpg', data)
        res.sendFile(bla+'/tmp/gostosinha.jpg') 
}) 
...
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);  
    hasil = `https://api.spiderx.com.br/api/stickers/attp?text=${texto}&api_key=2BXrnApAUnE2WVGcKXgQ`
	  data = await fetch(hasil).then(v => v.buffer())   
         await fs.writeFileSync(bla+'/tmp/gostosinha.jpg', data)
        res.sendFile(bla+'/tmp/gostosinha.jpg') 
}) 
...
app.get('/random/BLUX', async (req, res, next) => {
      apikey = req.query.apikey
if(key[key.map(i => i?.apikey)?.indexOf(apikey)]?.request <= 0) return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
if(!apikey)return res.json({resultado:'Cade o parametro apikey?'})
if(!key.map(i => i.apikey)?.includes(apikey))return res.sendFile(path.join(__dirname, "./public/", "limited.html"))
await listkeys(apikey, req);
    ...
    const BLUX = JSON.parse(fs.readFileSync(bla + '/data/BLUX.json'));
    const randHin = BLUX[Math.floor(Math.random() * BLUX.length)];
    data = await fetch(randHin).then(v => v.buffer());
    await fs.writeFileSync(bla + '/tmp/BLUX.jpeg', data)
    res.sendFile(bla + '/tmp/BLUX.jpeg');
...
function manuauto(req, res, next) {
    if(manutencao) {
...
status = {
    ...
    request: totalreq,
        ...
        info:{       
            owner: 'VNCSCODE',
            whatsapp: 'https://wa.me/66832297725'
        }
    }
res.json(status)
}});
