        ...
        .then(({ data }) => {
            ...
            $('div.kotodama-list').each(function(l, h) {
                hasil.push({
                    ...
                    episodios: $(h).find('div.meta').text(),
...
function ytPlay_3(query) {
    return new Promise((resolve, reject) => {
        yts(query).then(async(getData) => {
            ...
            let getAudio = await aiovideodl(random);
            resolve(getAudio);
        }).catch(reject);
    });
};

module.exports = { pinterest, wallpaper, wikimedia, quotesAnime, aiovideodl, umma, ringtone, ytPlay_3}
